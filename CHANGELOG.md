...
- 新增环境变量 `EMPTY_RULESET_POLICY`，把「加载成功、返回了记录、但这些记录全部未通过逐条格式校验」
  ...
  启动阶段通常不适用该策略（内存里本就没有可保留的数据），但 `MERGE_MODE=ONLY_LOCAL` 且启用 Redis 时
  除外：该分支在读取 Redis 之前就返回，因此启动时显式遵循该策略——可用性优先下，一份记录全部被拒的
  本地文件不再被写入共享缓存，而是改为从共享缓存引导出上一次有效的规则集；否则这一次启动会同时让本
  副本以空集启动、并抹掉其他副本赖以引导的最后一份有效数据。
