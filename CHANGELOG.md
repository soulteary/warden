...
- 修复 Redis 引导中 `Get` 与 `Exists` 之间的竞态。两者是两次往返，且发生在写入者选举之前：若键在
  `Get` 时不存在、而同伴副本随后发布了规则集，`Exists` 会如实返回「已存在」，本副本便会把写入前读到的
  空切片与写入后的存在性凑在一起，采纳空集并结束启动，对外提供空的允许列表直到下一个刷新周期。现改为
  确认存在后重新读取一次，使采纳的值不早于所依据的存在性；重读失败按未命中处理。
