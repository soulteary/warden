...
| 其他 | 空规则集策略 | — | `EMPTY_RULESET_POLICY` | `consistency-first`（另一取值：`availability-first`，详见[空规则集策略](#空规则集策略)） |
...
| DATA_DIR | 非空时须存在且为目录 |
| 空规则集策略 | 非空时须为 `consistency-first` 或 `availability-first`；非法取值直接启动失败（避免以为选了可用性优先、实际仍是一致性优先） |
...
Warden 的记录过滤分两层：**身份校验**（唯一标识冲突、缺失 `user_id`）作用于
整个集合，任何一条不合格都会让整次刷新失败并保留上一份有效数据；而**逐条
格式校验**（手机号、邮箱格式）在缓存层，它会静默丢弃不合格的记录、保留其余
记录。

两层叠加会产生一个特殊状态：**一次加载成功了、返回了记录，但这些记录一条也
没能进入生效集合**。最典型的成因是上游改了数据格式——比如把 `13800138000`
改成 `138-0013-8000`——这时上游自己完全"正常"，加载也"成功"，只是所有记录
在格式校验这一层被整批丢掉。

从 Warden 的角度，这个状态是**有歧义**的，两种读法在数据上完全无法区分：

| 读法 | 正确做法 |
|------|----------|
| "上游把所有人都撤权了" | 空集是正确结果，必须立刻生效 |
| "上游改了数据格式" | 空集是假象，绝不能生效 |

既然无法区分，就交给运维按部署形态选择。通过环境变量 `EMPTY_RULESET_POLICY`
配置：

| 取值 | 行为 | 适用场景 |
|------|------|----------|
| `consistency-first`（默认） | 应用并发布空集：内存缓存变空，共享 Redis 缓存同步为空集，本次刷新记为成功 | 上游是唯一权威数据源，**不一致比不可用更严重**：合法的全员撤权能立刻传播到所有副本；上游格式变更会立刻表现为"全部拒绝"，而不是被陈旧数据掩盖 |
| `availability-first` | 视为一次刷新失败：内存中保留上一次有效的规则集；本副本确实持有已知有效快照时才按同一份数据续期共享缓存，尚未成功引导时会重读 Redis 或跳过写入；失败计数递增、快照年龄继续增长、健康检查转为 degraded | **可用性损失大于错误放行的风险**：数据源不稳定且副本频繁重启时，一次上游抖动会清空所有副本，重启的副本连可引导的数据都没有 |

默认值 `consistency-first` 保持历史行为。它也是允许列表这类服务更合适的默认
偏置：继续放行数据源中已不存在的用户是**安全问题**，而拒绝用户是**可用性
问题**——后者响亮、显眼、能被立刻发现。

#### 不受策略影响的行为

- **数据源真的返回 0 条记录**不属于歧义场景，两种策略下都会照常生效。否则
  "撤销所有人"将永远无法执行。
- **启动阶段**（`loadInitialData`）通常先从共享 Redis 缓存引导；但一次临时读取失败
  并不意味着其中的最后有效集合已经消失。若随后从上游或本地加载到非空、但全部未
  通过格式校验的数据，可用性优先会在取得 Redis 写入者锁后重试读取：恢复后从共享
  集合启动；仍不可读时本副本保持为空，但绝不把空集写回 Redis，以免覆盖可能仍然
  存在的有效数据。

  `MERGE_MODE=ONLY_LOCAL` 会跳过常规的 Redis 优先读取，因此直接执行同一保护流程。
  一致性优先在所有启动路径仍保持历史行为：应用并发布逐条校验后的结果。
- **Redis 引导成功**会生成 `source=redis`、带加载时间和内容版本的 degraded 快照。
  因此严格远程模式可以从引导时开始计算 `SNAPSHOT_MAX_AGE`，而不是因为来源为
  `none` 在启动后立即返回 503。若启动始终未取得有效数据，首次后台刷新会再次
  尝试 Redis；仍然失败时不会把进程的零值空缓存写入共享缓存。若 Redis 中的旧版
  非空数据被当前格式校验全部过滤，仍按所选策略处理：一致性优先采纳生效空集；
  可用性优先拒绝其作为引导基线，并继续尝试配置的数据源。
- **启动时成功加载到真正的空集**会立即应用、记录快照并发布到 Redis；不会等到
  第一个后台周期才传播全员撤权。副本重启时，Redis `Get` 返回空切片后会再通过
  `Exists` 区分“已存储的合法空集”和“键不存在”：前者作为 `source=redis` 的已知
  有效快照直接生效，不会回退到可能陈旧的上游/本地数据；后者才继续尝试其他数据源。
  健康检查也通过快照来源区分合法空集与尚未加载数据。由于 `Get` 与 `Exists` 是两次
  往返、且发生在写入者选举之前，确认键存在之后会重新读取一次：否则同伴副本若在两次
  调用之间发布了规则集，本副本就会把「写入前读到的空切片」与「写入后观察到的存在性」
  凑在一起，对外提供空的允许列表，直到下一个刷新周期才恢复。
- **身份校验失败**（标识冲突、缺失 `user_id`）与该策略无关，在两种策略下都
  会保留上一份有效数据并记为刷新失败。如果一组记录同时存在身份冲突和逐条格式
  错误，身份完整性错误优先报告，不会被归类为 `all_records_rejected`。

#### 可观测性

| 策略 | 日志 | `warden_refresh_failures_total{reason}` |
|------|------|------------------------------------------|
| `consistency-first` | `WARN` + `loaded_count` / `policy` | 不计（本次刷新记为成功） |
| `availability-first` | `WARN` + `loaded_count` / `kept_count` / `has_known_good` / `consecutive_failures` | `reason="all_records_rejected"` |

可用性优先下不会推进快照版本，因此下个周期仍会判定为"数据有变化"并重新
尝试；上游恢复后，第一次成功刷新会清零失败计数并正常应用新数据。同时快照
年龄持续增长，超过 `SNAPSHOT_MAX_AGE` 后健康检查返回 503——即"仍在服务，但
数据已陈旧"是明确可见的，不会被静默掩盖。
...
- `status`（可选）：用户状态；省略时按 `"inactive"` 处理，必须显式设为 `"active"` 才允许继续认证
...
export WARDEN_HMAC_KEYS='{"key-id-1":"0123456789abcdef0123456789abcdef"}'  # 生产 secret 至少 32 字节
