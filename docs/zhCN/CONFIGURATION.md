...
| `availability-first` | 视为一次刷新失败：内存中保留上一次有效的规则集，共享缓存按同一份数据续期，失败计数递增、快照年龄继续增长、健康检查转为 degraded | **可用性损失大于错误放行的风险**：数据源不稳定且副本频繁重启时，一次上游抖动会清空所有副本，重启的副本连可引导的数据都没有 |
...
- **数据源真的返回 0 条记录**不属于歧义场景，两种策略下都会照常生效。否则
  "撤销所有人"将永远无法执行。
- **启动阶段**（`loadInitialData`）通常不适用该策略：此时进程内存里本就没有
  "上一次有效的规则集"可保留，两种策略结果相同。可用性优先想覆盖的重启场景由
  加载顺序本身解决——共享 Redis 缓存先于上游被读取，因此上游损坏期间重启的副本
  会从健康副本最后发布的数据集启动。

  **例外是 `MERGE_MODE=ONLY_LOCAL` 且启用了 Redis**（见上文，这是受支持的组合）：
  该分支在读取 Redis 之前就会返回，因此启动阶段会显式遵循该策略。若本地文件的
  记录全部未通过格式校验，可用性优先**不会**把空集写入共享缓存，而是改为从共享
  缓存引导出上一次有效的规则集。否则这一次启动会同时做两件事：让本副本以空集
  启动，并抹掉其他副本（以及此后每一次重启）赖以引导的最后一份有效数据——正是
  该策略要防止的场景。一致性优先在此保持历史行为：应用并发布空集。
...
| `availability-first` | `WARN` + `loaded_count` / `kept_count` / `consecutive_failures` | `reason="all_records_rejected"` |
