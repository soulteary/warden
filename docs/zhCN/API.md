...
下文列出的端点即 Warden 提供服务的全部路径。请求任何其他路径都会返回 `404 Not Found`
和一个 JSON 响应体，且不包含任何用户数据：
...
> **行为变更**：根路径 `/` 此前注册为子树通配模式，因此任何未匹配的路径（`/foo`、
> `/user/`、`/v1/`）都会由用户列表处理器响应并返回**完整允许列表**。现在 `/` 为精确匹配，
> 未匹配路径统一返回上面的 404。若此前依赖任意路径返回用户数据，请改用 `/`、`/data.json`
> 或 `/v1/users`。

注意 Go 的路由器会先做路径清理再匹配，因此 `/metrics/../user` 会被清理为 `/user` 并重定向，
不会落到 404 处理器。
...
| `ENVIRONMENT` | `/metrics` 默认行为 |
| --- | --- |
| `production` | 要求认证（与数据端点使用同一套认证方式） |
| `development`、`test`、未设置 | 允许匿名抓取 |

`WARDEN_METRICS_REQUIRE_AUTH` 可在两个方向上覆盖该默认值。对要求认证的部署进行匿名抓取会返回
`401 Unauthorized`。响应仅包含低基数、非敏感的指标序列：`endpoint` 与 `method` 标签按白名单归一化，
未识别的取值统一归入 `other`。
