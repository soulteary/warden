...
Warden 的文档与运行时消息提供 7 种语言，但它们**并非**同一维护标准。把它们当作等同维护，
正是 5 种语言悄悄落后 18 个翻译键和若干文档的原因。

**分级**

| 级别 | 语言 | 要求 |
| --- | --- | --- |
| 正本 | 英文（`enUS`）、简体中文（`zhCN`） | 必须与代码变更在同一个 PR 内更新。改变行为却未同步二者的 PR 视为未完成。 |
| 尽力而为 | `deDE`、`frFR`、`itIT`、`jaJP`、`koKR` | 允许滞后。每个滞后文档顶部带有横幅，提示读者以正本为准。 |

**运行时字符串不属于「尽力而为」。** `locales/*.json` 由 `go test ./locales/` 强制校验，
出现以下任一情况即失败：

- 缺少 `en.json` 中定义的键（或定义了 `en.json` 没有的键）；
- `printf` 占位符（`%s`、`%d`）的序列与英文原文不一致；
- 某个值与英文完全逐字节相同（即未翻译）。

缺失的键在运行时会回落到英文，因此不会有任何可见报错——这正是需要该校验的原因。新增面向
用户的消息时，请在同一次提交中把键加到**全部 7 个**语言文件里。若某个值确实应与英文相同
（外来词、协议名等），请在 `locales/locales_test.go` 的 `intentionallyIdentical` 中登记并
附注说明，而不是删掉该校验。

执行 `make docs-parity` 可查看各翻译文档相对 `enUS` 的滞后程度。
