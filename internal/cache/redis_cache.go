...
const (
	// RedisCacheKey 用于存储用户数据的 Redis key
	RedisCacheKey = "warden:users:cache"
	// RedisCacheVersionKey 用于存储缓存版本的 Redis key
	RedisCacheVersionKey = "warden:users:cache:version"
	// RedisCacheTTL Redis 缓存过期时间（1小时）
	RedisCacheTTL = 1 * time.Hour
	// RedisOperationTimeout Redis 操作超时时间
...
func (c *RedisUserCache) Set(users []define.AllowListUser) error {
	...
	pipe.Set(ctx, RedisCacheKey, data, RedisCacheTTL)
	// 版本号使用时间戳，确保每次更新都有新版本
	pipe.Incr(ctx, RedisCacheVersionKey)
	pipe.Expire(ctx, RedisCacheVersionKey, RedisCacheTTL)
...
func (c *RedisUserCache) Get() ([]define.AllowListUser, error) {
	...
	data, err := c.client.Get(ctx, RedisCacheKey).Bytes()
	if err != nil {
		if err == redis.Nil {
			// 缓存未命中，返回空切片
...
func (c *RedisUserCache) Exists() (bool, error) {
	...
	count, err := c.client.Exists(ctx, RedisCacheKey).Result()
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
func (c *RedisUserCache) GetVersion() (int64, error) {
	...
	version, err := c.client.Get(ctx, RedisCacheVersionKey).Int64()
	if err != nil {
		if err == redis.Nil {
...
func (c *RedisUserCache) Clear() error {
	...
	return c.client.Del(ctx, RedisCacheKey).Err()
}
